    ...
    <>
      <div className={cx('filter-book-container')}>
        <div className={cx('alert-container')}>
          {showAlert && (
            <Alert message={alertMessage} type={alertType} showIcon closable onClose={() => setShowAlert(false)} />
          )}
        </div>
        ...
        <div className={cx('results-content')}>
          <div className={cx('results-body')}>
            <div className={cx('results-list')}>
              <div className={cx('results-header')}>
                ...
                <div className={cx('results-dropdow')}>
                  ...
                  <Dropdown options={['Ngày đăng gần nhất', 'Filter 2', 'Filter 3']} onChange={handleSortChange} />
                </div>
              </div>
                ...
                currentItems.map((item) => (
                  <Link to={`/detail/${item.id}`} key={item.id} className={cx('book-item')}>
                    ...
                    <div className={cx('book-info')}>
                      ...
                      <div className={cx('action-dropdown')} ref={(el) => (dropdownRefs.current[item.id] = el)}>
                        <button
                        ...
                        {openDropdownId === item.id && (
                          <div className={cx('dropdown-menu')}>
                            ...
                            <button className={cx('dropdown-item')} onClick={(e) => handleAction('report', item.id, e)}>
                              Báo cáo
                            </button>
                            <button className={cx('dropdown-item')} onClick={(e) => handleAction('share', item.id, e)}>
                              Chia sẻ
                            </button>
                          </div>
                        )}
            ...
            <div className={cx('pagination')}>
              {Array.from({ length: Math.ceil(filteredResults.length / itemsPerPage) }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
