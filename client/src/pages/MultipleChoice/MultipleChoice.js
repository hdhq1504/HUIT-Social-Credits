        ...
        <div className={cx('questions-section')}>
          ...
          <div className={cx('questions-column')}>
            ...
            <ul className={cx('questions-list')}>
              ...
              <li>Thuật toán tìm kiếm nhị phân (Binary Search) có độ phức tạp thời gian trung bình là bao nhiêu?</li>
                ...
                {filteredTests.length === 0 ? (
                  ...
                  <div className={cx('tests-row')}>
                    {filteredTests.map((test, index) => (
                      <TestCard key={index} code={test.TenDe || `Mã đề #${test.id}`} testId={test.id} />
                    ))}
                  </div>
                )}
              </Box>
                  ...
                  {relatedSubjects.length > 0 ? (
                    ...
                    >
                      {relatedSubjects.map((subject, idx) => (
                        <TopicBox key={idx} title={subject.TenMH} description={subject.GioiThieu || 'Không có mô tả'} />
                      ))}
                    </Box>
                  ) : (
