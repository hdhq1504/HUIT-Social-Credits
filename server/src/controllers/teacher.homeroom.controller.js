...
export const getStudentScores = async (req, res) => {
  try {
    ...
    const activities = registrations.map(reg => {
      ...
      else if (reg.trangThai === 'VANG_MAT' || reg.trangThai === 'DA_HUY') status = 'REJECTED';
      else if (reg.trangThai === 'DANG_KY' || reg.trangThai === 'CHO_DUYET') status = 'PENDING';
...
export const exportClassScores = async (req, res) => {
  try {
    const { classId } = req.params;
    const teacherId = req.user.id;
    ...
    const data = students.map((student, index) => {
      ...
      student.dangKy.forEach(reg => {
        ...
        } else if (activity.nhomDiem === 'NHOM_2' || activity.nhomDiem === 'NHOM_3') {
      ...
      return {
        ...
        maSV: student.maSV,
        hoTen: student.hoTen,
        ngaySinh: student.ngaySinh ? new Date(student.ngaySinh).toLocaleDateString('vi-VN') : '',
    ...
    res.setHeader('Content-Disposition', `attachment; filename=DiemRenLuyen_${classInfo.maLop}.xlsx`);
    res.send(buffer);

  } catch (error) {
    console.error("Export class scores error:", error);
    res.status(500).json({ error: "Lỗi server khi xuất file" });
  }
};
