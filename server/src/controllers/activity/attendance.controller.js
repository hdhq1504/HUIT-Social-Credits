...
export const markAttendance = async (req, res) => {
  ...
  const { note, status, evidence, phase, faceDescriptor, faceDescriptors, faceError } = req.body || {};
  ...
  if (attendanceMethod === "photo") {
    ...
    if (!faceProfile) {
      return res.status(409).json({
        error: "Bạn chưa đăng ký khuôn mặt. Vui lòng đăng ký trong trang Thông tin sinh viên trước khi điểm danh.",
      });
    }
  }
  ...
  if (attendanceMethod === "photo") {
    ...
    if (directDescriptor) {
      ...
      descriptorCollection = [directDescriptor];
    } else {
      const collection = normalizeDescriptorCollection(faceDescriptors);
      if (collection.length) {
        descriptorCollection = collection;
        normalizedDescriptor = collection[0];
  ...
  await prisma.$transaction([
    ...
    prisma.diemDanhNguoiDung.create({
      data: {
        ...
        faceMeta,
        descriptors: descriptorCollection.length ? descriptorCollection : null,
      },
    }),
  ]);
