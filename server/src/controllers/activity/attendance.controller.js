...
export const markAttendance = async (req, res) => {
  ...
  if (attendanceMethod === "photo") {
    ...
    finalStatus = status === "present" ? "DA_THAM_GIA" : "VANG_MAT";
    entryStatus = finalStatus;
  }

  const registrationUpdate = {
    trangThai: attendanceMethod === "photo" ? registration.trangThai : finalStatus,
  ...
  if (isCheckout) {
    if (finalStatus === "DA_THAM_GIA") {
      ...
      emailMessageLines.push(`Điểm danh cho hoạt động "${activity.tieuDe}" ghi nhận bạn vắng mặt.`);
    }
  } else {
    if (attendanceMethod === "photo" && (faceMatchResult?.status === "REVIEW" || faceMatchResult?.status === "REJECTED")) {
      ...
      emailMessageLines.push(`Điểm danh đầu giờ cho hoạt động "${activity.tieuDe}" đã được ghi nhận thành công.`);
    }
  }
